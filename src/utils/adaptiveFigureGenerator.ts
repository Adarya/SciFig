...
import { DynamicFigureSizer, FigureDimensions } from './dynamicFigureSizer';

export interface AdaptivePlotlyData {
  x?: any[];
  y?: any[];
  z?: any[];
  type: string;
...
export interface AdaptivePlotlyLayout {
  title: {
    text: string;
    font: {
      size: number;
...
export interface AdaptiveFigureConfig {
...
export class AdaptiveFigureGenerator {
  private static readonly FONT_SETTINGS = {
    DPI_SCALE: 5, // 300 DPI equivalent
    FONT_FAMILY: 'Arial, sans-serif',
    LINE_WIDTH: 2,  // Thinner lines for publication
    MARKER_SIZE: 8,  // Smaller markers for publication
    GRID_COLOR: '#F0F0F0',  // Lighter grid
    TITLE_SIZE: 20,
    AXIS_TITLE_SIZE: 16,
    TICK_SIZE: 14,
    LEGEND_SIZE: 14
  ...
  static generateAdaptiveBoxPlot(
    data: any[],
    groupVar: string,
    valueVar: string,
  ...
  ): { data: AdaptivePlotlyData[]; layout: AdaptivePlotlyLayout; config: AdaptiveFigureConfig } {
    
    const groups = [...new Set(data.map(row => row[groupVar]))];
    const colors = this.getPublicationColors();
    
    // Calculate optimal dimensions based on data characteristics
    const dimensions = DynamicFigureSizer.calculateOptimalSize(
      data, groupVar, valueVar, 'box', customLabels
    );

    // Scale font sizes based on figure dimensions
    const fontSizes = this.calculateFontSizes(dimensions);
    
    const plotData: AdaptivePlotlyData[] = groups.map((group, index) => {
      ...
      return {
        ...
        marker: {
          ...
          size: this.FONT_SETTINGS.MARKER_SIZE,
          line: {
            color: '#000',
            width: 2
          }
        ...
        line: {
          color: '#000',
          width: this.FONT_SETTINGS.LINE_WIDTH
        }
      };
    });
    ...
    if (result.p_value < 0.05) {
      ...
      annotations.push({
        ...
        showarrow: false,
        font: { 
          size: fontSizes.annotation + 6, 
          color: '#000',
          family: this.FONT_SETTINGS.FONT_FAMILY
        }
      });
    ...
    const layout: AdaptivePlotlyLayout = {
      title: {
      ...
      xaxis: {
        ...
        showgrid: false,
        gridcolor: this.FONT_SETTINGS.GRID_COLOR,
        linecolor: '#000',
        linewidth: 2,
        tickfont: {
          size: fontSizes.tick,
    ...
    return {
      data: plotData,
      layout,
      config: this.getAdaptiveConfig('boxplot', dimensions)
    };
  }

  static generateAdaptiveBarPlot(
    data: any[],
    groupVar: string,
    valueVar: string,
    result: StatisticalResult,
    ...
    // Calculate optimal dimensions based on data characteristics
    const dimensions = DynamicFigureSizer.calculateOptimalSize(
      data, groupVar, valueVar, 'bar', customLabels
    );
    ...
    const plotData: AdaptivePlotlyData[] = [{
      ...
      error_y: {
        ...
        color: '#000',
        thickness: 3,
        width: Math.max(8, dimensions.width / 75) // Scale error bar width with figure size
      }
    }];
    ...
    return {
      data: plotData,
      layout,
      config: this.getAdaptiveConfig('barplot', dimensions)
    };
  }

  static generateAdaptiveSurvivalCurve(
    result: StatisticalResult,
    style: string = 'nature',
    customLabels?: { x?: string; y?: string; title?: string }
  ): { data: AdaptivePlotlyData[]; layout: AdaptivePlotlyLayout; config: AdaptiveFigureConfig } {
    ...
    // Calculate optimal dimensions for survival curves
    const mockData = times.map((time, i) => ({ time, event: events[i], group: groups?.[i] || 'All' }));
    const dimensions = DynamicFigureSizer.calculateOptimalSize(
      mockData, 'group', 'time', 'survival', customLabels
    );
    ...
    uniqueGroups.forEach((group, groupIndex) => {
      ...
      plotData.push({
        ...
        line: {
          ...
          width: Math.max(3, dimensions.width / 200), // Scale line width with figure size
    ...
    return {
      data: plotData,
      layout,
      config: this.getAdaptiveConfig('survival', dimensions)
    };
  }

  private static calculateFontSizes(dimensions: FigureDimensions): {
    title: number;
    axisTitle: number;
    tick: number;
    legend: number;
    ...
    // Scale font sizes based on figure dimensions
    const scaleFactor = Math.min(dimensions.width / 600, dimensions.height / 450);
    const clampedScale = Math.max(0.7, Math.min(1.5, scaleFactor));
    
    return {
      title: Math.round(this.FONT_SETTINGS.TITLE_SIZE * clampedScale),
      axisTitle: Math.round(this.FONT_SETTINGS.AXIS_TITLE_SIZE * clampedScale),
      tick: Math.round(this.FONT_SETTINGS.TICK_SIZE * clampedScale),
      legend: Math.round(this.FONT_SETTINGS.LEGEND_SIZE * clampedScale),
      annotation: Math.round(this.FONT_SETTINGS.TICK_SIZE * clampedScale)
    };
  }

  private static getAdaptiveConfig(figureType: string, dimensions: FigureDimensions): AdaptiveFigureConfig {
    return {
      displayModeBar: true,
      responsive: true,
      displaylogo: false,
      ...
      toImageButtonOptions: {
        format: 'png',
        filename: `adaptive_${figureType}_${Date.now()}`,
        height: dimensions.height,
        width: dimensions.width,
        scale: this.FONT_SETTINGS.DPI_SCALE
      }
    };
  }
